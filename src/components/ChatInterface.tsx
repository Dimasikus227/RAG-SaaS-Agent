...
export const ChatInterface: React.FC<ChatInterfaceProps> = ({
      ...
      const loadChatHistory = async () => {
        try {
          ...
          const {
            data,
            error
          } = await supabase.from('queries').select('query, response, created_at').eq('user_id', userId).order('created_at', {
            ascending: true
          });
          if (error) {
  ...
  return <div className="flex flex-col h-full">
      <div className="flex-1 overflow-y-auto p-4 space-y-4">
        {isLoadingHistory ? <div className="h-full flex flex-col items-center justify-center text-center p-4">
          ...
          </div> : messages.map(message => <div key={message.id} className={cn("flex items-start gap-3 max-w-3xl mx-auto", message.role === 'user' ? "flex-row-reverse self-end" : "")}>
              <div className={cn("flex h-8 w-8 rounded-md items-center justify-center", message.role === 'assistant' ? "bg-primary/10 text-primary" : "bg-muted")}>
                {message.role === 'assistant' ? <Bot className="h-5 w-5" /> : <User className="h-5 w-5" />}
              </div>
              <div className={cn("rounded-lg px-4 py-2 text-sm", message.role === 'assistant' ? "bg-muted" : "bg-primary text-primary-foreground")}>
                {message.content}
              </div>
            </div>)}
        {isLoading && <div className="flex items-start gap-3 max-w-3xl mx-auto">
      ...
      <div className="border-t p-4">
        <form onSubmit={handleSubmit} className="flex gap-2 max-w-3xl mx-auto">
          <Input placeholder="Введіть ваше повідомлення..." value={input} onChange={e => setInput(e.target.value)} className="flex-1" disabled={isLoading} />
